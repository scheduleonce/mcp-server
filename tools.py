...
def get_booking_time_slots(
    ...
    try:
        ...
        base_url = os.getenv("ONCEHUB_API_URL")
        if not base_url:
            return {
                "success": False,
                "error": "ONCEHUB_API_URL environment variable is not set",
                "user_friendly_error": "API URL not configured. Please set the ONCEHUB_API_URL environment variable.",
                "status_code": None,
...
def schedule_meeting(
    ...
    timeout: Annotated[int, Field(default=30, description="Maximum seconds to wait for the API response")] = 30,
    custom_fields: Annotated[Optional[dict], Field(default=None, description="Key-value pairs for the booking form. Example: {'company': 'Acme', 'interests': ['Pricing', 'Demo']}")] = None
) -> dict:
    """
    Books a meeting for a specific time slot. Before that you must identify a valid start_time using get_booking_time_slots before calling this tool.
    """
    try:
        # Get API key from context
        api_key = get_api_key_from_context()
